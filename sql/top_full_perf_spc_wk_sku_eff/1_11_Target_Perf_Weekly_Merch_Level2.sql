/*
1.Build the Perf_Weekly_Merch_Level2 table by aggregating the space and sales value at Department and Sub_Department Level 
2. Get the MERCH_LEVEL2_ID and STORE_ID by joining Merch_Level2 table and Store table 
*/

DELETE FROM `{{params.project_ID}}.{{params.Optumera_Target_DS}}.Perf_Weekly_Merch_Level2` WHERE true;
  ...
  `{{params.project_ID}}.{{params.Optumera_Target_DS}}.Perf_Weekly_Merch_Level2` ( 
    ...
    SELECT
     ...
     FROM 
      ...
      INNER JOIN
      `{{params.project_ID}}.{{params.Optumera_Target_DS}}.Merch_Level2` Merch
      ON
