/*
1.The Latest 5 Weeks of Performance Data which is deleted from Perf_Weekly_Merch_Level2 table.
2.The Latest 6 weeks of data(Updated 5 weeks of Data + Current Week Data) is loaded into Perf_Weekly_Merch_Level2 at Department,Sub_Department Level. 
*/
DELETE FROM `{{params.project_ID}}.{{params.Optumera_Target_DS}}.Perf_Weekly_Merch_Level2` where week_id in 
...
FROM 
	(
	SELECT week_id, Sales_org, Store_no,  Department, Sub_Department, 
	...
	Group by 
		week_id,  Sales_org, Store_no, Department, Sub_Department
	) Perf
INNER JOIN
	`{{params.project_ID}}.{{params.Optumera_Target_DS}}.Merch_Level2` Merch ON
	merch.ATTR_3 = CONCAT(Perf.Department,'|||',Perf.Sub_Department)
