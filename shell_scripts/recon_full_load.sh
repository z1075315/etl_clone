...
. ../config/etl_config.cfg
 ...
 echo "Reconciliation Script - Full load " 
 today="$(date '+%Y-%m-%d')"
 chk_flag='TRUE'
 echo $today
 logfile=$logfile_path"reconciliation_full_log_"$today".log"
 echo " `date -u` : Full Load Reconciliation  Validation Begins...." >> $logfile 

 tgt_table='staging.csv_banner'
 file_path=$etl_full_path'banner/*.csv'

 echo " `date -u` : Starting GS bucket and CSV table count check - Banner">> $logfile
 csv_count_check_fun
 echo " `date -u` : Completed GS bucket and CSV table count check  - Banner">> $logfile

 tgt_table='staging.csv_fiscal_week'
 file_path=$etl_full_path'fiscal_week/*.csv'

 echo " `date -u` : Starting GS bucket and CSV table count check - Fiscal Week">> $logfile
 csv_count_check_fun
 echo " `date -u` : Completed GS bucket and CSV table count check  - Fiscal Week">> $logfile
 ...
 if [ $chk_flag = 'FALSE' ]; then
       echo " `date -u` : Full Load Count check validation failed.... Investigate ETL Pipeline-I & CSV Table Load process " >> $logfile
       #set alert to notify support
       exit 1
 else
       echo " `date -u` : Full Load Count check validation completed successfully.. Proceeding with Stage table Load"  >> $logfile
       exit 0
 fi
